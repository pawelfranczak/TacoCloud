...
public class AccountService extends CrudServiceImpl<Account, Long>{
	...
	public BigDecimal getSumOfAll() {
		List<Account> findByOwner = getAll();
		return (findByOwner.stream().map(Account::getBalance).reduce(BigDecimal.ZERO, BigDecimal::add));
	}
	
	public BigDecimal getSumOfAllActiveAndRelevantForSum() {
		List<Account> findByOwner = getAllActive();
		return (findByOwner.stream().filter(Account::getTakeIntoTotalSum).map(Account::getBalance).reduce(BigDecimal.ZERO, BigDecimal::add));
	}
